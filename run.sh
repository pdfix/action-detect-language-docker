...
print_help() {
    ...
    echo
    echo "Options:"
    echo "  --input <input>         Path to the input PDF or text file"
    echo "  --output <output>       Path the output PDF or text file"
...
docker_cmd+=" -it $IMAGE lang-detect -i \"$DATA_IN/$INPUT_FILE\" -o \"$DATA_OUT/$OUTPUT_FILE\""
